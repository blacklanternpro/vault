...
**Month-as-OS is dead. The globe / beige seal shell is dead. Kanban cards and harvest amber are dead.** Keep **SPECK**. The field is a composed desktop canvas: split-page PIPE binder, unix NEST, DUMP tape, one-line dock. Persistence is SPECK source (`speck-os-v1`), not a task CRDT, not JSON-as-database.
...
**Tonight-of-that-future-run:** stay inside the language. Refine the binder, nest, dump, PLACE. Do not ship another stack of components and call it done.
...
3. **Current shell** — `src/App.tsx` hosts `<Field />`. Runtime is `src/speck`. Split-page PIPE. No grain/scan. No globe. The React calendar/todo/scratch components are a corpse; do not restore them. The month pack is not the destination.
...
5. **Boards** — [cassette futurism ui](https://au.pinterest.com/search/pins/?q=cassette%20futurism%20ui&rs=typed) is the world. Lotus split-page is PIPE geometry — restamp onto cobalt, leave leather/amber.
...
- Field `#000000`
- Paper `#F4F4F0`
- Cobalt `#0000FF` — spine, invert, dossier
- Urgent `#FF2B2B` — strike / URGENT only

Helvetica Neue / Helvetica / Arial. Radius 0. No shadows. No grain. No globe. No amber.

---

## Composition law

One field. Desktop. One dock.

1. PIPE: FOCUS | spine | GATEWAY. Shovel, not drag-cards. Empty ledger rows stay.
2. NEST pit. CLIP stem → TASK.
3. DUMP tape.
4. PLACE / GLYPH. Overlap legal.
5. HIT TASK → cobalt dossier overlay from the row.

You may refine density. You may not add hardware bays, card grids, a second input, the globe, month-as-OS, or PROJECTS/ROSTER/FLEET/LOGS tabs.
